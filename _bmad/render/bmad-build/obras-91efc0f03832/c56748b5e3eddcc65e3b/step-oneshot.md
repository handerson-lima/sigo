...
If `{story_key}` is not empty and `/Users/usuario/obras/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/c56748b5e3eddcc65e3b/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'dispatch'` and `status: 'draft'`. Go back to `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/c56748b5e3eddcc65e3b/step-02-plan.md` step 6.
...
If `{story_key}` is not empty and `/Users/usuario/obras/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/c56748b5e3eddcc65e3b/sync-sprint-status.md` with `{target_status}` = `review`.
