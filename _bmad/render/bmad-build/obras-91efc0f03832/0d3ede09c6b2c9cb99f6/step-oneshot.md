...
- Speak in `pt-br`. Write files in `pt-br`.
...
If `{story_key}` is not empty and `/Users/usuario/obras/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/0d3ede09c6b2c9cb99f6/sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'dispatch'` and `status: 'draft'`. Go back to `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/0d3ede09c6b2c9cb99f6/step-02-plan.md` step 6.
...
If a layer needs subagents and you cannot launch them, write the full prompt for each layer under `/Users/usuario/obras/_bmad-output/implementation-artifacts` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `/Users/usuario/obras/_bmad-output/implementation-artifacts/deferred-work.md`:
...
If `{story_key}` is not empty and `/Users/usuario/obras/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `/Users/usuario/obras/_bmad/render/bmad-build/obras-91efc0f03832/0d3ede09c6b2c9cb99f6/sync-sprint-status.md` with `{target_status}` = `review`.
